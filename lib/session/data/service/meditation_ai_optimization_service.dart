...
class MeditationAIOptimizationService {
  ...
  late AllMeditationsRepository _meditationsRepository;
  late MeditationAIOptimizationRepository _meditationAIOptimizationRepository;
  late SettingsModel _settingsModel;
  final SettingsRepository _settingsRepository;

  final int minimumDataPointsPrediction = 600;
  ...
  Future<int> getTotalDataCount() async {
    int dataPoints = 0;
    List<MeditationModel>? allMeditations = await _meditationsRepository.getAllMeditation();
    if(allMeditations != null){
      for(MeditationModel meditationModel in allMeditations){
        for(SessionParameterModel sessionParameter in meditationModel.sessionParameters){
          dataPoints += sessionParameter.heartRates.length;
        }
      }
    }
    return dataPoints;
  ...
  Future<void> trainModel() async {
    if(dataForTrainingAvailable()){
      http.Response response = await _meditationAIOptimizationRepository.trainModel(createTrainingRequestData());
      // TODO verify status code + error handling
      // save number of already trained data points
      _settingsModel.trainedDataPoints = _settingsModel.trainedDataPoints + minimumDataPointsPrediction;
      _settingsRepository.saveSettings(_settingsModel);
    }
  }

  Future<PredictionResponseModel?> predict(PredictionRequestModel currentParameters) async {
    // check whether ML model already exists
    if(mlModelIsAvailable()){
      http.Response response = await _meditationAIOptimizationRepository.predict(currentParameters);
      if (response.statusCode == 200) {
        // If the server did return a 200 OK response,
        // then parse the JSON.
        return PredictionResponseModel.fromJson(jsonDecode(response.body) as Map<String, dynamic>);
      } else {
  ...
  MLTrainModel createTrainingRequestData(){
  ...
  static Future<MeditationAIOptimizationService> create(
    ...
    MeditationAIOptimizationService meditationAIOptimizationService = MeditationAIOptimizationService(settings);
